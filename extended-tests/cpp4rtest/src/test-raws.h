...
context("raws-C++") {
  ...
  test_that("writable::raws(initializer_list<named_arg>)") {
    ...
    cpp4r::writable::raws x({"one"_nm = x1, "two"_nm = x2, "three"_nm = x3});
    expect_true(x.named());
    expect_true(x["one"] == 1);
    expect_true(x["two"] == 2);
  ...
  test_that("writable::raws(initializer_list<named_arg>) type check") {
    using namespace cpp4r::literals;
    expect_error_as(cpp4r::writable::raws({"one"_nm = true}), cpp4r::type_error);
    expect_error_as(cpp4r::writable::raws({"one"_nm = R_NilValue}), cpp4r::type_error);

    // `as_sexp()` turns this into an `INTSXP`, which is not compatible
    expect_error_as(cpp4r::writable::raws({"one"_nm = 1}), cpp4r::type_error);
  }

  test_that("writable::raws(initializer_list<named_arg>) length check") {
    using namespace cpp4r::literals;
    SEXP x = PROTECT(Rf_allocVector(RAWSXP, 2));
    expect_error_as(cpp4r::writable::raws({"x"_nm = x}), std::length_error);
    UNPROTECT(1);
  }

  test_that("writable::raws(initializer_list<uint_8>)") {
    cpp4r::writable::raws x({1, 2, 255});
    expect_true(x[0] == 1);
    expect_true(x[1] == 2);
    expect_true(x[2] == 255);
  }
