...
context("logicals-C++") {
  ...
  test_that("writable::logicals(initializer_list<named_arg>)") {
    ...
    cpp4r::writable::logicals x({"one"_nm = x1, "two"_nm = x2, "three"_nm = x3});
    expect_true(x.named());
    expect_true(x["one"] == cpp4r::r_bool(false));
    expect_true(x["two"] == cpp4r::r_bool(true));
    expect_true(x["three"] == cpp4r::r_bool(NA_LOGICAL));

    // From booleans
    cpp4r::writable::logicals y({"one"_nm = true, "two"_nm = false, "three"_nm = true});
    expect_true(y.named());
    expect_true(y["one"] == cpp4r::r_bool(true));
    expect_true(y["two"] == cpp4r::r_bool(false));
    expect_true(y["three"] == cpp4r::r_bool(true));
  ...
  test_that("writable::logicals(initializer_list<named_arg>) type check") {
    using namespace cpp4r::literals;
    expect_error_as(cpp4r::writable::logicals({"one"_nm = 1}), cpp4r::type_error);
    expect_error_as(cpp4r::writable::logicals({"one"_nm = R_NilValue}),
                    cpp4r::type_error);
  }
  ...
  test_that("writable::logicals(initializer_list<named_arg>) length check") {
    ...
    expect_error_as(cpp4r::writable::logicals({"x"_nm = x}), std::length_error);
    UNPROTECT(1);
  }

  test_that("writable::logicals(initializer_list<r_bool>)") {
    cpp4r::writable::logicals x(
        {cpp4r::r_bool(true), cpp4r::r_bool(false), cpp4r::r_bool(NA_INTEGER)});
    expect_true(x[0] == cpp4r::r_bool(true));
    expect_true(x[1] == cpp4r::r_bool(false));
    expect_true(x[2] == cpp4r::r_bool(NA_INTEGER));

    // This works due to implicit conversion of `bool` to `r_bool`
    cpp4r::writable::logicals y({true, false, false});
    expect_true(y[0] == cpp4r::r_bool(true));
    expect_true(y[1] == cpp4r::r_bool(false));
    expect_true(y[2] == cpp4r::r_bool(false));

    // This works due to implicit conversion of `Rboolean` to `r_bool`
    cpp4r::writable::logicals z({TRUE, FALSE, FALSE});
    expect_true(z[0] == cpp4r::r_bool(true));
    expect_true(z[1] == cpp4r::r_bool(false));
    expect_true(z[2] == cpp4r::r_bool(false));
  }
  ...
  test_that("is_na(r_bool)") {
    ...
    cpp4r::r_bool y = NA_LOGICAL;
    expect_true(cpp4r::is_na(y));
  }

  test_that("FALSE and false") {
    cpp4r::writable::logicals x{FALSE};
    expect_true(x.size() == 1);
    expect_true(x[0] == FALSE);

    cpp4r::writable::logicals y{false};
