...
This generally means you'll need to edit Roxygen comments in an `.R`, not a `.Rd` file. You can find the `.R` file that generates the `.Rd` by reading the comment in the first line.
...
*   Please use `make format` and `styler::style_pkg()` to style 'C++' and 'R' code before sending a PR.
