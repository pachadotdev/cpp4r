...
class r_vector {
 ...
 public:
  ...
  class const_iterator {
   ...
   private:
    ...
    static bool use_buf(bool is_altrep) noexcept;
 ...
 private:
  ...
  /// Implemented in specialization
  static underlying_type* get_p(bool is_altrep, SEXP data) noexcept;
  /// Implemented in specialization
  static underlying_type const* get_const_p(bool is_altrep, SEXP data) noexcept;
  /// Implemented in specialization
...
inline r_vector<T>::r_vector(std::initializer_list<T> il)
    : r_vector(static_cast<SEXP>(writable::r_vector<T>(il))) {}
...
inline void r_vector<T>::push_back(T value) {
  if (__builtin_expect(length_ >= capacity_, 0)) {
    reserve(capacity_ == 0 ? 1 : capacity_ * 2);
  }

  if (__builtin_expect(data_p_ != nullptr, 1)) {
