...
register <- function(path = NULL, quiet = !is_interactive(), extension = c(".cpp", ".cc")) {
  ...
  has_testthat <- any(vapply(src_files, function(f) {
    any(grepl("<testthat.h>", readLines(f, warn = FALSE), fixed = TRUE))
  }, logical(1)))
  testthat_decl <- if (has_testthat) 'extern "C" SEXP run_testthat_tests(SEXP);\n\n' else ""
...
generate_cpp_functions <- function(funs, package = "cpp4r") {
  ...
  out <- mapply(function(file, return_type, cpp_name, real_params, package, name, sexp_params, calls) {
    paste0(
      "// ", basename(file), "\n",
  ...
  }, funs$file, funs$return_type, funs$cpp_name, funs$real_params, funs$package, funs$name, funs$sexp_params, funs$calls, SIMPLIFY = TRUE)
...
get_call_entries <- function(path, funs, package, fun_guards = list()) {
  ...
  sym    <- paste0("_", package, "_", funs$name)

  entries <- as.character(mapply(function(s, n, nm) {
    line  <- sprintf('    {"%s", (DL_FUNC) &%s, %d},', s, s, n)
    guard <- fun_guards[[nm]]
    if (!is.null(guard)) paste0(guard, "\n", line, "\n#endif") else line
  }, sym, n_args, funs$name, SIMPLIFY = TRUE, USE.NAMES = FALSE))

  # Detect testthat runner: if any src file includes <testthat.h>, add its entry.
  src_files <- list.files(file.path(path, "src"),
                          pattern = "\\.(cpp|cc|h|hpp)$",
                          full.names = TRUE)
  has_testthat <- any(vapply(src_files, function(f) {
