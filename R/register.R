...
register <- function(path = NULL, quiet = !is_interactive(), extension = c(".cpp", ".cc")) {
  ...
  source_files <- src_files[grepl("\\.(cpp|cc)$", src_files)]
  generated_source <- file.path(path, "src", paste0("cpp4r", extension))
  source_files <- source_files[normalizePath(source_files, mustWork = FALSE) !=
    normalizePath(generated_source, mustWork = FALSE)]

  header_includes <- character()
  if (!length(source_files)) {
    main_header <- file.path(path, "src", "main.h")
    header_files <- if (file.exists(main_header)) {
      main_header
    } else {
      src_files[grepl("\\.(h|hpp)$", src_files)]
    }
    src_prefix <- paste0(normalizePath(file.path(path, "src"), mustWork = FALSE), .Platform$file.sep)
    header_files <- sub(src_prefix, "", normalizePath(header_files, mustWork = FALSE), fixed = TRUE)
    header_files <- gsub("\\\\", "/", header_files)
    header_includes <- paste0(
